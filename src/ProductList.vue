<template>
  <div id="products" class="row list-group">
    <div v-for="product in products" class="item col-xs-4" :key="product.id">
      <div class="thumbnail">
        <router-link
...
<script>
  import {ADD_PRODUCT_TO_CART} from './mutation-types.js';
  ...
  export default {
    ...
    created() {
      this.$http.get('products')
                .then(response => response.json(),
                      response => alert("Could not fetch products!"),)
    ...
    methods: {
      [ADD_PRODUCT_TO_CART](product, quantity) {
        this.$store.dispatch(ADD_PRODUCT_TO_CART, {product, quantity})
